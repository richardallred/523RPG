...
dojo.declare('myapp.Dookenstein', [dijit._Widget, dijit._Templated], {
    ...
    postMixInProperties: function() {
		...
		//initialize safe combinations array
		this.combinationItems = new Array();
		this.combinationValues = new Array();
	...
	loadPageTextAndChoices: function(data) {
		...
		for (i = 0; i < dataSplit.length; i++) {
			...
			if (this.COMMENTDELIMITER != "" && dataSplit[i].indexOf(this.COMMENTDELIMITER) == 0) {
			...
			else if (dataSplit[i].indexOf('COMBINATION:') != -1) {
				//Parse a safecracking combination item
				splitResult = dataSplit[i].split('COMBINATION:')[1];
				var added = {
					name: 'Error with combination item initialization - no name specified',
					value: -1
				}
				...
				this.combinationItems[this.combinationItems.length] = added;
			}
				...
				*/
				...
				for(p=0; p<specialPageArray.length; p++){
									...
									if (enemyHit && aliveEnemies[f].health > 0 && !aliveEnemies[f].stunned) {
										...
										if (currentArmor != "None" && damageTaken > 0) {
											if (Math.random() <= currentArmor.probability/100) {
												damageReduced = Math.round(Math.random()*(currentArmor.defense))
												if (damageReduced == 0) {
													damageReduced = 1;
												}
					...
					//LOCKPICK: num of tumblers
						...
						this.message = specialPageArray[specialPageArray.length-2];
							...
							if (choiceNum==1) {
								...
								if (this.currentPushes!=this.maxPushes) {
									...
									if (this.currentPushes == 1) {
										...
										this.lockString = "You have pushed the tumbler " + (this.currentPushes) + " times.  ";
									}
									this.message = this.message + "<br>HINT: " + this.hint;
									this.lockString = this.lockString + "<br>HINT: " + this.hint;
							...
							//Choose to check current pushes
								...
								//Incorrect Pushes for the that tumbler
									...
									//Otherwise tell them its wrong and try again
										...
										if (this.currentPushes < this.tumblers[this.currentTumbler-1]) {
											this.message="Tumbler " + (this.currentTumbler) + " grinds against the lock mechanism before falling to the ground.  You judge that you pushed it too low.";
										} else {
											this.message="Tumbler " + (this.currentTumbler) + " grinds against the lock mechanism before falling to the ground.  You judge that you pushed it too high.";
										}
										this.currentPushes = 0;
										this.lockString = this.message;
										this.message=this.message+"<br>You are currently picking tumbler "+(this.currentTumbler) +" of "+this.maxTumblers;
										this.message=this.message+"<br>You have "+this.maxWrong+" attempts left to check tumblers in this lock.";
										this.lockString = this.lockString+"<br>You have "+this.maxWrong+" attempts left to check tumblers in this lock.";
										this.message=this.message+"<br>HINT: " + this.hint;
										this.lockString = this.lockString + "<br>HINT: " + this.hint;
									}
								} else {
					...
					//SAFECRACK:pagenum.  Redirect to pagenum if the player aborts the safecracking attempt
						...
						if(this.inSafeCracking==0 || (!crackedSafe && choiceNum == 1)){
							...
							for (x = 0; x < this.combinationItems.length; x++) {
								for (y = 1; y < this.inventory.length; y++) {
									if (this.combinationItems[x].name == this.inventory[y]) {
										this.hasCombo++;
									}
								}
							}
							...
							if (this.hasCombo > 0 && this.safeDisplay.length < this.combinationItems.length) {
								...
								for (i = 0; i < this.hasCombo; i++) {
									this.safeDisplay[i] = 'empty';
								}
								for (i = 0; i < this.hasCombo; i++) {
									rand = Math.floor(Math.random()*this.hasCombo);
									counter = 0;
									while (this.safeDisplay[rand] != 'empty' && counter < 999) {
										rand = Math.floor(Math.random()*this.hasCombo);
										counter++;
									}
	...
	runJSonic: function() {
		if ((this.inCombat == 0 && this.inSafeCracking == 0 && this.inLockPicking == 0) || this.reread == 1) {
			this.js.stop();
		}
